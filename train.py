...
from utils_regulate_patches import DIV2KTrainSet, DIV2KValidSet, FeatureExtractor, TV_Loss, save_plot
...
#=========================================Resume Running Functions===========================================#
    ...
    if opt.resume:
        # If opt.resume ends with ".pth", treat it as a full path to a checkpoint file
        if os.path.isfile(opt.resume) or opt.resume.endswith(".pth"):
            check_point_path = opt.resume
        else:
            # Otherwise, treat it as an epoch number and construct the path
