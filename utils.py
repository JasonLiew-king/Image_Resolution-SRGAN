...
class DIV2KTrainSet(Dataset):
    def __init__(self, hr_dir, lr_dir, patch_size=128, stride=98, upscale_factor=4):
        super().__init__()
        self.hr_dir = hr_dir
        self.lr_dir = lr_dir
    ...
    def _extract_patches(self, image):
        """Extracts patches from the HR image using a sliding window approach."""
    ...
    def _extract_all_patches(self):
        """Extract patches from HR images and generate corresponding LR patches."""
        all_patches = []
        for hr_filename in self.hr_files:
            hr_image = Image.open(hr_filename).convert("RGB")  
            hr_patches = self._extract_patches(hr_image)  # Extract HR patches

            for hr_patch in hr_patches:  # Extract HR patches
                lr_patch = self.lr_transform(hr_patch)  # Downscale HR patch to LR
                all_patches.append((hr_patch, lr_patch))  # Store matched HR & LR patches
        return all_patches
    ...
    def __getitem__(self, index):
        hr_patch, lr_patch = self.patches[index]
        
        # Apply transformations
        hr_patch = self.hr_normalize(hr_patch)  # Normalize HR
        return hr_patch, lr_patch  # LR is already a tensor
